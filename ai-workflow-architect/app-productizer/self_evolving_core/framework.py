...
try:
    ...
    from bridge_integration import BridgeIntegration, publish_mutation_event, publish_fitness_update, publish_system_evolution
...
class EvolvingAIFramework:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # Bridge integration for unified system
        self.bridge_integration: Optional[BridgeIntegration] = None
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Bridge integration (if available)
            if BRIDGE_AVAILABLE:
                try:
                    import asyncio
                    ...
                    self.bridge_integration = loop.run_until_complete(
                ...
                logger.info("ℹ️ Bridge integration not available")
    ...
    async def _initialize_bridge_integration(self) -> Optional[BridgeIntegration]:
        """Initialize bridge integration asynchronously"""
        try:
            from bridge_integration import create_bridge_integration
            integration = await create_bridge_integration()
            
            # Register event handlers for cross-system events
            from shared_types import EventType as BridgeEventType
            
            async def handle_workflow_event(event):
                """Handle workflow events from Bridge API"""
                logger.info(f"Received workflow event: {event.type}")
                # Process workflow optimization requests, etc.
            
            integration.register_event_handler(BridgeEventType.WORKFLOW_STARTED, handle_workflow_event)
            integration.register_event_handler(BridgeEventType.WORKFLOW_COMPLETED, handle_workflow_event)
            integration.register_event_handler(BridgeEventType.AGENT_CONFIGURED, handle_workflow_event)
    ...
    def _publish_to_bridge(self, event_type, payload):
        """Publish event to bridge API if available"""
        if self.bridge_integration:
            try:
                ...
                if loop.is_running():
                    ...
                    asyncio.create_task(self.bridge_integration.publish_event(event_type, payload))
                else:
                    # Run in the loop if not running
                    loop.run_until_complete(self.bridge_integration.publish_event(event_type, payload))
            except Exception as e:
                logger.error(f"Failed to publish to bridge: {e}")
    ...
    def propose_mutation(self, mutation: Mutation) -> Dict[str, Any]:
        ...
        if self.autonomy.should_auto_approve(mutation):
            ...
            if result.success:
                ...
                # Publish to bridge API
                if BRIDGE_AVAILABLE:
                    from shared_types import EventType as BridgeEventType
                    self._publish_to_bridge(BridgeEventType.MUTATION_APPLIED, {
    ...
    def get_fitness(self) -> FitnessScore:
        ...
        if BRIDGE_AVAILABLE:
            from shared_types import EventType as BridgeEventType
            self._publish_to_bridge(BridgeEventType.FITNESS_CALCULATED, {
